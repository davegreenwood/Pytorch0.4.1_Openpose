...
class Openpose(object):
    def __init__(self, arch='posenet', weights_file=None, training=False):
        self.arch = arch
        if weights_file:
        ...
        #         CocoDataset(coco_train, params['insize']),
        #         params['batch_size'],
        #         shuffle=True, pin_memory=False,
        #         num_workers=params['num_workers'])
        #     self.val_loader = DataLoader(
        #         CocoDataset(
        #             coco_val, params['insize'],
        #             mode='val'),
        #         params['batch_size'],
    ...
    def board_scalars(self, key, loss, paf_log, heatmap_log):
        ...
        for stage, (paf_loss, heatmap_loss) in enumerate(
                zip(paf_log, heatmap_log)):
            self.writer.add_scalar(
                '{}_paf_loss_stage{}'.format(key, stage),
                paf_loss, self.step)
            self.writer.add_scalar(
                '{}_heatmap_loss_stage{}'.format(key, stage),
                heatmap_loss, self.step)
        ...
        with torch.no_grad():
            for imgs, pafs, heatmaps, ignore_mask in iter(self.val_loader):
                imgs, pafs, heatmaps, ignore_mask = imgs.to(
                    self.device), pafs.to(
                    self.device), heatmaps.to(
                    self.device), ignore_mask.to(
                    self.device)
    ...
    def save_state(self, val_loss, to_save_folder=False, model_only=False):
        ...
        torch.save(
            ...
            save_path /
            ('model_{}_val_loss:{}_step:{}.pth'.format(
                time, val_loss, self.step)))
        if not model_only:
            torch.save(
                self.optimizer.state_dict(),
                save_path /
                ('optimizer_{}_val_loss:{}_step:{}.pth'.format(
                    time, val_loss, self.step)))
    ...
    def find_lr(self,
        ...
        losses = []
        log_lrs = []
        for i, (imgs, pafs, heatmaps, ignore_mask) in tqdm(
                    ...
                    with torch.no_grad():
                        for i in range(20):
                            ...
                            img_path = os.path.join(
                                params['coco_dir'],
                                'val2017', self.val_loader.dataset.coco.
                                loadImgs([img_id])[0]['file_name'])
    ...
    def compute_peaks_from_heatmaps(self, heatmaps):
        ...
        for i, heatmap in enumerate(heatmaps):
            ...
            peaks_with_score = [
                (i,) + peak_pos + (heatmap[peak_pos[1], peak_pos[0]],) for peak_pos in peaks]

            peaks_id = range(peak_counter, peak_counter +
        ...
        all_peaks = np.array(
            [peak
             for peaks_each_category in all_peaks
             for peak in peaks_each_category])
        return all_peaks

    def compute_candidate_connections(
            self, paf, cand_a, cand_b, img_len, params):
        ...
        candidate_connections = sorted(
            candidate_connections, key=lambda x: x[2],
            reverse=True)
    ...
    def compute_connections(self, pafs, all_peaks, img_len, params):
        ...
        for i in range(len(params['limbs_point'])):
            ...
            # example: [<JointType.Neck: 1>, <JointType.RightWaist: 8>]
    ...
    def grouping_key_points(self, all_connections, candidate_peaks, params):
        ...
        for l, connections in enumerate(all_connections):
            ...
            for ind_a, ind_b, score in connections[:, :3]:
                ...
                if joint_found_cnt == 1:
                    ...
                    membership = ((found_subset_1 >= 0).astype(
                        int) + (found_subset_2 >= 0).astype(int))[:-2]
                    if not np.any(
                            membership == 2):  # merge two subsets when no duplication
                        # default is -1
                        found_subset_1[:-2] += found_subset_2[:-2] + 1
                        found_subset_1[-2:] += found_subset_2[-2:]
                        found_subset_1[-2:] += score
                        subsets = np.delete(
                            subsets, joint_found_subset_index[1],
                            axis=0)
                    else:
        ...
        keep = np.logical_and(subsets[:, -1] >=
                              params['n_subset_limbs_thresh'],
                              subsets[:, -2] / subsets[:, -1] >=
                              params['subset_score_thresh'])
    ...
    def compute_limbs_length(self, joints):
        ...
        for i, joint_indices in enumerate(params["limbs_point"]):
            if joints[joint_indices[0]] is not None and joints[joint_indices[1]] is not None:
                limbs.append(
                    [joints[joint_indices[0]],
                     joints[joint_indices[1]]])
                limbs_len[i] = np.linalg.norm(
                    joints[joint_indices[1]][: -1] -
    ...
    def compute_unit_length(self, limbs_len):
        ...
        if len(np.nonzero(non_zero_limbs_len)[0]) > 0:
            ...
            unit_length = np.sum(
                base_limbs_len[non_zero_limbs_len] /
                limbs_len_ratio[non_zero_limbs_len]) / len(
                np.nonzero(non_zero_limbs_len)[0])
        else:
            limbs_len_ratio = np.array(
                [2.2, 1.7, 1.7, 2.2, 1.7, 1.7, 0.6, 0.93, 0.65, 0.85, 0.6,
                 0.93, 0.65, 0.85, 1, 0.2, 0.2, 0.25, 0.25])
            non_zero_limbs_len = limbs_len > 0
            unit_length = np.sum(
                limbs_len[non_zero_limbs_len] /
                limbs_len_ratio[non_zero_limbs_len]) / len(
                np.nonzero(non_zero_limbs_len)[0])
    ...
    def crop_person(self, img, person_pose, unit_length):
        top_joint_priority = [4, 5, 6, 12, 16, 7, 13, 17,
                              8, 10, 14, 9, 11, 15, 2, 3, 0, 1, sys.maxsize]
        bottom_joint_priority = [9, 6, 7, 14, 16, 8, 15,
                                 17, 4, 2, 0, 5, 3, 1, 10, 11, 12, 13, sys.maxsize]
        ...
        top = (
            top_pos - top_padding_radio[top_joint_index] * unit_length).astype(int)
        bottom = (
            bottom_pos + bottom_padding_radio[bottom_joint_index] * unit_length).astype(int)
        bbox = (left, top, right, bottom)
    ...
    def crop_hands(self, img, person_pose, unit_length):
        ...
        if person_pose[JointType.LeftHand][2] > 0:
            ...
            if person_pose[JointType.LeftElbow][2] > 0:
                direction_vec = person_pose[JointType.LeftHand][:-
                                                                1] - person_pose[JointType.LeftElbow][:-1]
                crop_center += (0.3 * direction_vec).astype(crop_center.dtype)
            hand_img, bbox = self.crop_around_keypoint(
                img, crop_center, unit_length * 0.95)
            hands["left"] = {
                "img": hand_img,
                "bbox": bbox
            }
        ...
        if person_pose[JointType.RightHand][2] > 0:
            ...
            if person_pose[JointType.RightElbow][2] > 0:
                direction_vec = person_pose[JointType.RightHand][:-
                                                                 1] - person_pose[JointType.RightElbow][:-1]
                crop_center += (0.3 * direction_vec).astype(crop_center.dtype)
            hand_img, bbox = self.crop_around_keypoint(
                img, crop_center, unit_length * 0.95)
            hands["right"] = {
    ...
    def detect_precise(self, orig_img):
        ...
        for scale in params['inference_scales']:
            ...
            img = cv2.resize(
                orig_img,
                (math.ceil(orig_img_w * multiplier),
                 math.ceil(orig_img_h * multiplier)),
                interpolation=interpolation)
            padded_img, pad = self.pad_image(
                img, params['downscale'],
                (104, 117, 123))
            ...
            pafs_sum += cv2.resize(tmp_paf, (orig_img_w,
                                             orig_img_h), interpolation=interpolation)
            ...
            tmp_heatmap = cv2.resize(
                ...
                fy=params['downscale'],
                interpolation=interpolation)
            tmp_heatmap = tmp_heatmap[:padded_img.shape[0]-pad[0],
                                      :padded_img.shape[1]-pad[1], :]
            heatmaps_sum += cv2.resize(tmp_heatmap, (orig_img_w,
                                                     orig_img_h), interpolation=interpolation)


        self.pafs = (
            pafs_sum / len(params['inference_scales'])).transpose(2, 0, 1)
        self.heatmaps = (
            heatmaps_sum / len(params['inference_scales'])).transpose(2, 0, 1)
        ...
        all_connections = self.compute_connections(
            self.pafs, self.all_peaks, orig_img_w, params)
        subsets = self.grouping_key_points(
            all_connections, self.all_peaks, params)
    ...
    def detect(self, orig_img, precise=False):
        ...
        map_w, map_h = self.compute_optimal_size(
